...
let CfiveComponent = class CfiveComponent {
    constructor() {
        this.cimal = 6;
    }
    ngOnInit() {
...
let CfourComponent = class CfourComponent {
    constructor() {
    }
    ngOnInit() {
    }
...
let ConeComponent = class ConeComponent {
    constructor() { }
    ngOnInit() {
...
let CthreeComponent = class CthreeComponent {
    constructor() { }
    ngOnInit() {
    }
};
...
let CtwoComponent = class CtwoComponent {
    // json-server --watch db.json
    // COUNSTRUCTOR
    // ng build --prod
    constructor(fb, route, employeeService, router) {
        this.fb = fb;
        this.route = route;
    ...
    onContactPrefernceChange(selectedValue) {
    ...
    onSubmit() {
        ...
        if (this.employee.id) {
            this.employeeService.updateEmployee(this.employee).subscribe(() => this.router.navigate(['edit']), (err) => console.log(err));
        }
    }
    // ADD BUTTON FUNCTION SERVICE API iiiiiiiiiiiiiiiiiiiiiiiiiii
    onadd() {
        this.mapFormValuesToEmployeeModel();
        this.employeeService.addEmployee(this.employee).subscribe(() => this.router.navigate(['list']), (err) => console.log(err));
    }
...
let ListEmployeesComponent = class ListEmployeesComponent {
    ...
    editButtonClick(employeeId) {
        this.router.navigate(['/list', employeeId]);
    }
    deleteButtonClick(employeeid) {
        this.employeeService.deleteEmployee(employeeid).subscribe(() => this.router.navigate(['lisklk']), (err) => console.log(err));
    }
    ngOnInit() {
        // CALLING GET API FROM SERVICE
        this.employeeService.getEmployees().subscribe((employeeList) => this.employees = employeeList, (err) => console.log(err));
    }
};
